...
void initialize_policy_tree() {
  ...
  // Roleplaying
  {
    ...
    section_root->add_child("^cRoleplaying is consent-based.^n All players involved must consent to a scene. While assumed consent holds for ordinary scenes, anything with conflict, significant relationship drama (cheating etc), spying, adult situations, aggressive actions or speech, etc needs to be pre-approved by participants. Lack of approval/consent requires that the situation be unwound to a mutually-agreeable point. ^WWe recommend using the TELL command to obtain consent so that it can be proven.^n");
    section_root->add_child("^cActive consent required.^n If at any time a player becomes uncomfortable and makes this known (via the ^WCONSENT^n command, osays, tells, or any other in-game medium), the scene must immediately be stopped. If you cannot reach a consensus with your roleplay partner(s) about the outcome of a scene, contact RP staff for assistance. If you find yourself asking 'should we stop this scene?', the answer is usually 'yes'.");
